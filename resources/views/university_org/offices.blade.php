    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($organizations as $org)
            <div class="bg-white rounded-lg shadow-md p-6 flex flex-col items-center text-center relative">
                ...
                @if($org->orgAdmin && ! $org->orgAdmin->invitation_active)
                    <div class="mb-2 inline-flex items-center gap-2">
                        @if($org->orgAdmin->invitation_pending)
                            <span class="px-2 py-1 rounded-full bg-yellow-100 text-yellow-800 text-xs font-semibold">Invitation Pending</span>
                        @elseif($org->orgAdmin->invitation_expired)
                            <span class="px-2 py-1 rounded-full bg-red-100 text-red-800 text-xs font-semibold">Invitation Expired</span>
                        @endif
                    </div>

                    @if($org->orgAdmin->invitation_expired)
                        <form action="{{ route('university_org.offices.resendInvite', $org->id) }}" method="POST" class="mt-2">
                            @csrf
