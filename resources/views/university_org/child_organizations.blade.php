...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <a href="{{ route('university_org.reports', ['school_year_id' => $selectedSY->id,'semester_id' => $selectedSem->id]) }}"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="p-4 bg-indigo-50 rounded shadow text-center">
            <p class="text-sm text-gray-600">Payment Completion</p>
            <p class="text-2xl font-bold">{{ $paymentCompletionRate }}%</p>
        </div>
    </div>

    {{-- Child Organization Cards --}}
    @if($childOrgs->isEmpty())
        <p class="text-gray-500">No child organizations found.</p>
    @else
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($childOrgs as $org)
        <div class="p-4 bg-white rounded shadow-sm flex flex-col sm:flex-row sm:items-start gap-4">
            @if($org->logo)
            ...
            <div class="flex-1 flex flex-col justify-between text-sm">
                <div class="space-y-1">
                    ...
                    <p class="text-gray-500 text-xs">College: {{ $org->college?->name ?? 'N/A' }}</p>
                    <p class="text-gray-500 text-xs">Admin: {{ $org->orgAdmin?->first_name ?? 'N/A' }} {{ $org->orgAdmin?->last_name ?? '' }}</p>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2 mt-2 text-xs">
                    ...
                    <div class="bg-green-50 p-2 rounded text-center">
                        ...
                        <p class="font-semibold">₱{{ number_format($org->total_payments, 2) }}</p>
                    </div>
                ...
                <div class="mt-3">
                    <a href="{{ route('university_org.child_org_fees', ['org_id' => $org->id, 'school_year_id' => $selectedSY->id, 'semester_id' => $selectedSem->id]) }}"
