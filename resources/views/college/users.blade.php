...
@section('content')
@php
    $activeTab = request()->get('tab', 'accounts'); 
@endphp
...
@if($activeTab === 'college')
    <div class="grid grid-cols-1 md:grid-cols-1 gap-5 mb-4">
        <div class="bg-white p-8 rounded shadow">
            <div class="flex flex-col md:flex-row md:items-center md:space-x-8 space-y-6 md:space-y-0">
                <!-- College Logo -->
                <form id="logoForm" action="{{ route('college.info.updateLogo') }}" method="POST" enctype="multipart/form-data" class="relative flex-shrink-0">
                    @csrf
                    @method('PUT')
                    <label for="college_logo" class="block text-gray-700 font-medium mb-2 text-center md:text-left">College Logo</label>
                    
                    <label for="college_logo" class="cursor-pointer relative block">
                        @if($currentCollege?->logo)
                            <img src="{{ asset('storage/' . $currentCollege->logo) }}" 
                                alt="Logo" 
                                class="h-40 w-40 object-cover rounded border border-gray-300 mx-auto md:mx-0">
                        @else
                            <div class="h-40 w-40 flex items-center justify-center rounded border border-gray-300 bg-gray-100 text-gray-500 mx-auto md:mx-0">
                                No Logo
                            </div>
                        @endif
                ...
                <form id="nameForm" action="{{ route('college.info.updateName') }}" method="POST" class="flex-1" onsubmit="return confirmNameUpdate()">
                    @csrf
                    @method('PUT')
                    <label for="college_name" class="block text-gray-700 font-medium mb-2 text-center md:text-left">College Name</label>
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <button type="submit" class="px-4 py-3 bg-red-800 text-white rounded hover:bg-red-700 whitespace-nowrap">
                            Update
                        </button>
                    </div>
                </form>
