...
<div x-data="{ search: '', rowsVisible: {{ $payments->count() }}, clear() { 
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
        <div class="relative w-full md:w-1/3">
            ...
            <button type="button" x-show="search.length > 0" @click="clear()" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-sm font-bold">
                &times;
            </button>
        </div>

        <button @click="openFilter = true" class="bg-gray-100 hover:bg-gray-200 text-gray-700 inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition">
