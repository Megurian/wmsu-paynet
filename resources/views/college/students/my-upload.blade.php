        ...
        <div @click.away="showModal = false" class="bg-white rounded-lg shadow-lg w-full max-w-lg relative p-6">
            ...
            <form method="POST" action="{{ route('college.students.my-upload.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="student_id" value="{{ old('student_id') }}" required class="w-full rounded border px-3 py-2 text-sm  @error('student_id') border-red-500 focus:ring-red-200 @enderror" placeholder="Enter student ID">
                ...
                <td colspan="3">
                    <form method="POST" action="{{ route('college.students.my-upload.readd', $student->id) }}" class="flex gap-2 items-center">
                        ...
                        <select name="course_id" class="border rounded px-2 py-1 text-sm" required {{ $enrollment ? 'disabled' : '' }}>
                            <option value="">Select Course</option>
                            @foreach($courses as $course)
                            <option value="{{ $course->id }}" @if($enrollment && $course->id == $enrollment->course_id) selected
                        ...
                        <select name="year_level_id" class="border rounded px-2 py-1 text-sm" required {{ $enrollment ? 'disabled' : '' }}>
                            ...
                            @foreach($years as $year)
                            <option value="{{ $year->id }}" @if($enrollment && $year->id == $enrollment->year_level_id) selected
                        ...
                        <select name="section_id" class="border rounded px-2 py-1 text-sm" required {{ $enrollment ? 'disabled' : '' }}>
                            <option value="">Select Section</option>
                            @foreach($sections as $section)
                            <option value="{{ $section->id }}" @if($enrollment && $section->id == $enrollment->section_id) selected
