...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            <p><span class="font-semibold">Adviser:
                </span> {{ $studentEnrollments->first()->adviser?->first_name ?? '—' }}
                {{ $studentEnrollments->first()->adviser?->middle_name ?? '—' }}
                {{ $studentEnrollments->first()->adviser?->last_name ?? '—' }}
            </p>
    ...
    <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100 text-gray-700">
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-5 py-3">
                    @php
                    $latestPayment = $payments->filter(function($p) use ($fee) {
                    return $p->fees->contains($fee->id);
                    })->sortByDesc('created_at')->first();
                    @endphp

                    {{ $latestPayment?->created_at?->format('F d, Y') ?? '—' }}
